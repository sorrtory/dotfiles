...
| --- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 01  | `host-deps`       | Installs the host commands later phases need, using the distro package manager, and refreshes `ca-certificates` until a CA bundle sits where Nix looks for one. The [script itself](scripts/bootstrap/01-host-deps.sh) is the authoritative dependency list. | sudo password                                                            |
...
| 10  | `virtualization`  | Installs distro QEMU/KVM, libvirt, virt-manager and UEFI firmware; enables local services and the default VM network, and binds its bridge to the firewalld `libvirt` zone so guests can reach DHCP. | sudo password |
