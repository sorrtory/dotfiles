...
- The operator reshaped the interface twice. Where it landed: a path is
  optional everywhere and means `./Vault`, never `~/Vault`; `unlock` and `lock`
  are the gocryptfs routine, `open` is unlock plus Nautilus and `notes` is
  unlock plus Obsidian; errors are visible from a keybinding. `~/Vault` is only
  a convention, written down once in the GNOME module for `<Super>n`. Vault
