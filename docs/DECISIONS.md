...
The user environment has one base, the `nixos-26.05` pin, and one escape hatch beside it: the `nixpkgs-unstable` input, read through `unstablePkgs`. It is not a second base. A module keeps taking its packages from `pkgs` and names `unstablePkgs.<name>` only for the specific package stable cannot serve, so the set of packages ahead of the release is exactly the set someone wrote down. Four qualify today, for two reasons: sing-box needs the namespace support absent from the stable pin, while `gallery-dl`, `spotdl`, and `gdown` track changing services. A stale copy of one of those downloaders can fail on a site or public link that used to work. The stable pin has gdown 5.2.2, while the unstable pin has 6.2.0 with newer Drive link handling, folder and export support, and a browser-cookie option. Anything taken from here owes a paragraph below saying which of those it is; "newer is better" is not a reason, since the release pin is the thing being traded away. A single input also means one lock entry moving on `nix flake update`, rather than several branch pins drifting apart.
...
No downloader is a wrapped program of the local proxy, because none needs to be. The wrapper exists for Codex and Claude Code, which have no proxy flag or setting at all and read only the standard environment variables, so the sole lever is the environment they are started in. Every downloader here takes a proxy argument instead, which is the smaller tool: `download` reads `PROXY` and translates it into each backend's own spelling — `--proxy` for yt-dlp, gallery-dl, spotdl and gdown, `--all-proxy` for aria2c. `PROXY` is defined once by the local-proxy module, from the same binding the wrappers use, so a command cannot drift from the endpoint the service actually listens on. It is deliberately not `HTTP_PROXY` or `ALL_PROXY`: nothing reads `PROXY` implicitly, so naming it in the session tunnels nothing that did not ask. spotdl is the one backend whose flag is not enough: `--proxy` covers only its audio download, while its Spotify and YouTube Music lookups read the environment, and both are region-blocked on a direct connection. So `download` puts `PROXY` into spotdl's own environment as `HTTP(S)_PROXY`, scoped to that one process, and `modules/scripts.nix` patches spotapi — the Spotify scraper spotDL uses by default, whose TLS client ignored the environment entirely — to honour it.


Fetching and converting are two commands, `download` and `convert-to`. `download [--as TYPE|EXTENSION] [--using BACKEND] URL... [-- BACKEND_OPTIONS...]` chooses a backend from the source URL, then asks it for an output result. `--as` does not silently select a backend, so a Pinterest video can still be fetched by gallery-dl; `--using` overrides a wrong guess or handles a site not in the table. The table records operator defaults rather than copying the downloaders' supported-site lists: YouTube/Vimeo/TikTok/Twitch are yt-dlp video, SoundCloud is yt-dlp audio, Spotify and `saved` are spotdl MP3, Pinterest/Instagram/Imgur/Pixiv/ArtStation/DeviantArt/Flickr/e621/Danbooru are gallery-dl published media, Google Drive is gdown, Yandex.Disk and direct file URLs are aria2c. An unknown HTTP site opens an fzf backend picker only on a terminal; a noninteractive caller must use `--using`. The command prints the chosen result and backend before starting. An explicit result that the chosen backend cannot produce fails before any download.

Backend-specific options follow `--`, which leaves `--as`, `--using`, `--cookies`, and `--cookies-from-browser` unambiguous. Multiple URLs can share one invocation when they resolve to one backend and one default result; gdown's CLI accepts only one URL. Spotify cannot yield an untouched source stream, so MP3 is its default, and `--spotify-meta` opts a YouTube link into spotdl's metadata matching. A public Yandex.Disk share link is resolved through its download API just before aria2c starts; that API request uses `PROXY` too. gdown handles Google's confirmation and cookie flow itself rather than copying its changing Drive scraping logic into the shell script. These URL defaults deliberately replace the original type-first interface, which forced backend choice from the requested format and made gallery-dl video links awkward.
...
Two more findings are recorded here because they contradict what the flags look like they do. `yt-dlp --remux-video` fails outright when the target container does not support the codec rather than falling back to re-encoding, so `download --as mp4` always uses `--recode-video`, which is a no-op when the source already fits; `convert-to mp4` may remux only because it reads the codecs with `ffprobe` first. And `gallery-dl --proxy ""` is not a direct connection, as above.
