...
GPU drivers are host integration that Home Manager nonetheless has to supply. A Nix closure's Vulkan and EGL loaders read the distro's ICD manifests but cannot open the driver libraries they name, so on a non-NixOS distro every GPU context fails and a graphical Nix package silently degrades to software output — verified on both the staging VM and this host, where the loader skipped every Ubuntu ICD in turn. `targets.genericLinux.gpu` builds matching drivers from Nixpkgs; the symlink at `/run/opengl-driver` that makes them visible needs root, so activation only checks it and prints the command, keeping working rule 7 intact. The privileged step is explicit host setup like Docker's, documented in the README rather than run for the operator. This is not MPV-specific and lives in its own module, but MPV is what surfaced it: `mpv.conf` selects `vo=gpu-next` with `gpu-api=vulkan`, and the Anime4K shaders exist only on the GPU path.
