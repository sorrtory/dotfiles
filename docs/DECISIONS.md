...
Every device's configuration is kept as ciphertext so any machine can recover its own, but a machine decrypts only the one it uses, and decrypts it as `wg0.conf`. `wg-quick` takes the interface name from the basename, so naming the secret for the interface rather than for the device means every machine brings up `wg0` from whichever device file is its own; a firewall rule or script naming the interface then holds everywhere. The configurations are per-device identities rather than per-provider, so materializing all of them everywhere would let one compromised machine impersonate every device on the network, and would buy nothing: a laptop has no use for the phone's key. Selecting per machine is done by hand until host parameterization exists.
